...
#exclude missing timing
dat7 <- dat6[!is.na(dat6$DX_RAD_STARTED_DAYS) & !is.na(dat6$DX_SURG_STARTED_DAYS),]
dim(dat7)

#exclude patients diagnosed before 2012 
dat8 <- dat7[(dat7$YEAR_OF_DIAGNOSIS < 2012),]
dim(dat8)
...
N1Mi_ct <- ctree(Surv(followup_time, censor) ~ as.factor(),data=N1mi_P)
plot(GBSG2ct)
