...
NODAL[NODAL$AGE <60 & NODAL$AGE >=50,'AGE_Recode'] = "2. 50-60"
NODAL[NODAL$AGE <70 & NODAL$AGE >=60,'AGE_Recode'] = "3. 60-70"
NODAL[NODAL$AGE >=70,'AGE_Recode']                 = "4. > or = 70"
...
NODAL$P_S_H = NULL
NODAL[(NODAL$interval_S_H >=0 & NODAL$interval_S_H <30) & !is.na(NODAL$interval_S_H),'P_S_H'] = "1. <1 month"
NODAL[(NODAL$interval_S_H >=30) & !is.na(NODAL$interval_S_H),'P_S_H'] = "2. > or = 1 month"
NODAL[is.na(NODAL$interval_S_H),'P_S_H'] = "3. Not Applicable"

# Interval from Hormones to RT 
# if >0, then hormones before RT
NODAL$interval_H_RT = NODAL$R_Date - NODAL$H_Date
NODAL$P_H_RT = NULL
NODAL[(NODAL$interval_H_RT >=0 & NODAL$interval_H_RT <30) & !is.na(NODAL$interval_H_RT),'P_H_RT'] = "1. <1 month"
NODAL[(NODAL$interval_H_RT >=30) & !is.na(NODAL$interval_H_RT),'P_H_RT'] = "2. > or = 1 month"
NODAL[is.na(NODAL$interval_H_RT),'P_S_H'] = "3. Not Applicable"
# Stratify into those who start hormones before RT vs. during AND after RT

# period until RT
NODAL$P_RT = NULL
NODAL[(NODAL$interval >=0 & NODAL$interval <84),'P_RT'] = "1. <12 weeks"
NODAL[(NODAL$interval >=84),'P_RT'] = "2. > or = 12 weeks"
...
listVars<-(c("AGE_Recode","RACE_Recode",'CDCC_TOTAL_Recode',"TNM_PATH_T_Recode",'TNM_PATH_N_Recode','HISTOLOGY_Recode',"ERSTATUS","GRADE_Recode","LYMPH_VASCULAR_INVASION_Recode",'HORM_RT_SEQ','P_H_RT','P_S_H','RX_SUMM_SURGICAL_MARGINS_Recode','RX_SUMM_CHEMO_Recode','RX_SUMM_HORMONE_Recode','FACILITY_TYPE_CD_Recode','CROWFLY_Recode',"YEAR_OF_DIAGNOSIS",'INSURANCE_STATUS_Recode','INCOME_Recode','EDUCATION_Recode'))


catVars<-(c("AGE_Recode","RACE_Recode",'CDCC_TOTAL_Recode',"TNM_PATH_T_Recode",'TNM_PATH_N_Recode','HISTOLOGY_Recode',"ERSTATUS","GRADE_Recode","LYMPH_VASCULAR_INVASION_Recode",'HORM_RT_SEQ','P_H_RT','P_S_H', 'RX_SUMM_SURGICAL_MARGINS_Recode','RX_SUMM_CHEMO_Recode','RX_SUMM_HORMONE_Recode','FACILITY_TYPE_CD_Recode','CROWFLY_Recode',"YEAR_OF_DIAGNOSIS",'INSURANCE_STATUS_Recode','INCOME_Recode','EDUCATION_Recode'))


table1 <- CreateTableOne(vars = listVars, data = NODAL, factorVars = catVars, strata = 'P_RT',includeNA=TRUE)
 write.csv(print(table1),'H:\\Gerber\\NCDB/Timing_TableOne_2_13_2017_Timing.csv')
...
coxmodel <- coxph(Surv(followup_time, censor)~ P_RT,data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))


coxmodel <- coxph(Surv(followup_time, censor)~ P_H_RT,data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))


coxmodel <- coxph(Surv(followup_time, censor)~ P_S_H,data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))


coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(AGE_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(RACE_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(CDCC_TOTAL_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(droplevels(TNM_PATH_T_Recode)),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(ERSTATUS),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(GRADE_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(HISTOLOGY_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(LYMPH_VASCULAR_INVASION_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(LN_SAMPLING_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(RX_SUMM_SURGICAL_MARGINS_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(RX_SUMM_CHEMO_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(RX_SUMM_HORMONE_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(FACILITY_TYPE_CD_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(CROWFLY_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(INSURANCE_STATUS_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(INCOME_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(EDUCATION_Recode),data=NODAL)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))
...
write.csv(UNI_Results,'H:/Gerber/Timing_2_13_2017_UnivariateResults.csv')
write.csv(UNI_Results[UNI_Results[,4]<0.05,],'H:/Gerber/Timing_2_13_2017_Significant_UnivariateResults.csv')
...
NODAL_Modified <- NODAL[(NODAL$INCOME_Recode !='Unknown') & (NODAL$EDUCATION_Recode != 'Unknown') ,]
...
+ as.factor(P_RT):as.factor(droplevels(TNM_PATH_T_Recode))
+ as.factor(P_RT):as.factor(droplevels(TNM_PATH_N_Recode))
+ as.factor(P_RT):as.factor(AGE_Recode)
#+ as.factor(P_RT):as.factor(ERSTATUS)
+ as.factor(P_RT):as.factor(RX_SUMM_HORMONE_Recode)
,data=NODAL_ERPOS)
...
coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(P_RT)
+ as.factor(AGE_Recode)
+ as.factor(CDCC_TOTAL_Recode)
+ as.factor(droplevels(TNM_PATH_T_Recode))
+ as.factor(droplevels(TNM_PATH_N_Recode))
...
glmmodel <- glm( as.factor(P_RT) ~ as.factor(AGE_Recode), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# # sjp.glm(glmmodel)

glmmodel <- glm(as.factor( P_RT)~ as.factor(RACE_Recode), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)

glmmodel <- glm(as.factor( P_RT)~ as.factor(CDCC_TOTAL_Recode), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)

glmmodel <- glm(as.factor( P_RT)~ as.factor(droplevels(TNM_PATH_T_Recode)), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)

glmmodel <- glm(as.factor( P_RT)~ as.factor(droplevels(TNM_PATH_N_Recode)), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)

glmmodel <- glm(as.factor( P_RT)~ as.factor(ERSTATUS), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)


glmmodel <- glm(as.factor( P_RT)~ as.factor(HISTOLOGY_Recode), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)


glmmodel <- glm(as.factor( P_RT)~ as.factor(GRADE_Recode), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)

glmmodel <- glm(as.factor( P_RT)~ as.factor(LYMPH_VASCULAR_INVASION_Recode), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)

glmmodel <- glm(as.factor( P_RT)~ as.factor(HORM_RT_SEQ), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)

glmmodel <- glm(as.factor( P_RT)~ as.factor(RX_SUMM_SURGICAL_MARGINS_Recode), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)

glmmodel <- glm(as.factor( P_RT)~ as.factor(RX_SUMM_HORMONE_Recode), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)

glmmodel <- glm(as.factor( P_RT)~ as.factor(FACILITY_TYPE_CD_Recode), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)

glmmodel <- glm(as.factor( P_RT)~ as.factor(CROWFLY_Recode), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)

glmmodel <- glm(as.factor( P_RT)~ as.factor(INSURANCE_STATUS_Recode), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))
# sjp.glm(glmmodel)

glmmodel <- glm(as.factor( P_RT)~ as.factor(INCOME_Recode), family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))

glmmodel <- glm(as.factor( P_RT)~ EDUCATION_Recode, family='binomial',data=NODAL)
UNI_Results_2 = rbind(UNI_Results_2,cbind(exp(glmmodel$coefficients),exp(confint(glmmodel)),coef(summary(glmmodel))[,4]))

UNI_Results_2<-round(UNI_Results_2,3)
colnames(UNI_Results_2) = c('HR','2.5% CI','97.5% CI','p')
# write all univariate and only the significant variables
write.csv(UNI_Results_2,'H:/Gerber/Timing_2_13_2017_OR_UnivariateResults.csv')
write.csv(UNI_Results_2[UNI_Results_2[,4]<0.05,],'H:/Gerber/Timing_2_13_2017_Significant_OR_UnivariateResults.csv')

################## Multivariate ANOVA ##################
 
 catVars<-(c("AGE_Recode","RACE_Recode",'CDCC_TOTAL_Recode',"TNM_PATH_T_Recode",'TNM_PATH_N_Recode','HISTOLOGY_Recode',"ERSTATUS","GRADE_Recode","LYMPH_VASCULAR_INVASION_Recode",'HORM_RT_SEQ', 'RX_SUMM_SURGICAL_MARGINS_Recode','RX_SUMM_HORMONE_Recode','FACILITY_TYPE_CD_Recode','CROWFLY_Recode',"YEAR_OF_DIAGNOSIS",'INSURANCE_STATUS_Recode','INCOME_Recode','EDUCATION_Recode'))

 
glmmodel <- glm(as.factor(P_RT) ~ as.factor(AGE_Recode)+
+ as.factor(RACE_Recode)
+ as.factor(CDCC_TOTAL_Recode)
+ as.factor(droplevels(TNM_PATH_T_Recode))
+ as.factor(droplevels(TNM_PATH_N_Recode))
...
MULTI_RESULTS <- cbind(exp(glmmodel$coefficients),exp(confint.default(glmmodel)),coef(summary(glmmodel))[,4])

write.csv(MULTI_RESULTS[MULTI_RESULTS[,4]<0.05,],'H:/Gerber/Timing_2_13_2017_Significant_OR_MULTIvariateResults.csv')
...
+ as.factor(FACILITY_TYPE_CD_Recode)
 ...
 + as.factor(INCOME_Recode)
+ as.factor(INSURANCE_STATUS_Recode)
+ as.factor(CROWFLY_Recode),data=NODAL_test)

results = rbind(results,c(exp(coxmodel$coefficients[6]),exp(confint(coxmodel))[6,],table(NODAL_test$P_RT2)[2]))
print(cutoff)
}
...
plot(cutoff_l,results2[,1],
    ...
    main="Hazard Ratio of Death with Various Cutoffs for RT Initiation Time \n Taken from Cox Model of patients \n Adjusting for All Significant Variables"
)
...
save(splits,file='H:\\Gerber\\NCDB\\splits_1000.R')
...
################# SENSITIVITY ANALYSIS WITH ER STATUS ##############
NODAL_ERSTATUS <- NODAL[NODAL$ERSTATUS == '1. Positive' | NODAL$ERSTATUS == '2. Negative',]

# descriptive statistics
listVars<-(c("P_RT","AGE_Recode","RACE_Recode",'CDCC_TOTAL_Recode',"TNM_PATH_T_Recode",'TNM_PATH_N_Recode','HISTOLOGY_Recode',"GRADE_Recode","LYMPH_VASCULAR_INVASION_Recode",'HORM_RT_SEQ','P_H_RT','P_S_H','RX_SUMM_SURGICAL_MARGINS_Recode','RX_SUMM_CHEMO_Recode','RX_SUMM_HORMONE_Recode','FACILITY_TYPE_CD_Recode','CROWFLY_Recode',"YEAR_OF_DIAGNOSIS",'INSURANCE_STATUS_Recode','INCOME_Recode','EDUCATION_Recode'))


catVars<-(c("P_RT","AGE_Recode","RACE_Recode",'CDCC_TOTAL_Recode',"TNM_PATH_T_Recode",'TNM_PATH_N_Recode','HISTOLOGY_Recode',"GRADE_Recode","LYMPH_VASCULAR_INVASION_Recode",'HORM_RT_SEQ','P_H_RT','P_S_H', 'RX_SUMM_SURGICAL_MARGINS_Recode','RX_SUMM_CHEMO_Recode','RX_SUMM_HORMONE_Recode','FACILITY_TYPE_CD_Recode','CROWFLY_Recode',"YEAR_OF_DIAGNOSIS",'INSURANCE_STATUS_Recode','INCOME_Recode','EDUCATION_Recode'))


table1 <- CreateTableOne(vars = listVars, data = NODAL_ERSTATUS, factorVars = catVars, strata = 'ERSTATUS',includeNA=TRUE)
 write.csv(print(table1),'H:\\Gerber\\NCDB/Timing_ER_TableOne_2_13_2017_Timing.csv')
...
coxmodel <- coxph(Surv(followup_time, censor)~ P_RT,data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))


coxmodel <- coxph(Surv(followup_time, censor)~ P_H_RT,data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))


coxmodel <- coxph(Surv(followup_time, censor)~ P_S_H,data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))


coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(AGE_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(RACE_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(CDCC_TOTAL_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(droplevels(TNM_PATH_T_Recode)),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(ERSTATUS),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(GRADE_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(HISTOLOGY_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(LYMPH_VASCULAR_INVASION_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(LN_SAMPLING_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(RX_SUMM_SURGICAL_MARGINS_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(RX_SUMM_CHEMO_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(RX_SUMM_HORMONE_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(FACILITY_TYPE_CD_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(CROWFLY_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(INSURANCE_STATUS_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(INCOME_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(EDUCATION_Recode),data=NODAL_ERPOS)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

UNI_Results<-round(UNI_Results,3)
colnames(UNI_Results) = c('HR','2.5% CI','97.5% CI','p')
# write all univariate and only the significant variables
write.csv(UNI_Results,'H:/Gerber/Timing_2_13_2017_ERUnivariateResults.csv')
write.csv(UNI_Results[UNI_Results[,4]<0.05,],'H:/Gerber/Timing_2_13_2017_ERSignificant_UnivariateResults.csv')
...
NODAL_ERPOS_Modified <- NODAL_ERPOS[(NODAL_ERPOS$INCOME_Recode !='Unknown') & (NODAL_ERPOS$EDUCATION_Recode != 'Unknown') ,]
...
+ as.factor(FACILITY_TYPE_CD_Recode)
 ...
 + as.factor(INCOME_Recode)
+ as.factor(INSURANCE_STATUS_Recode)
+ as.factor(CROWFLY_Recode),data=NODAL_ERPOS_Modified)
...
coxmodel <- coxph(Surv(followup_time, censor)~ P_RT,data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))


coxmodel <- coxph(Surv(followup_time, censor)~ P_H_RT,data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))


coxmodel <- coxph(Surv(followup_time, censor)~ P_S_H,data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))


coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(AGE_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(RACE_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(CDCC_TOTAL_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(droplevels(TNM_PATH_T_Recode)),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(ERSTATUS),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(GRADE_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(HISTOLOGY_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(LYMPH_VASCULAR_INVASION_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(LN_SAMPLING_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(RX_SUMM_SURGICAL_MARGINS_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(RX_SUMM_CHEMO_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(RX_SUMM_HORMONE_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(FACILITY_TYPE_CD_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(CROWFLY_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(INSURANCE_STATUS_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(INCOME_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(EDUCATION_Recode),data=NODAL_ERNEG)
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))

UNI_Results<-round(UNI_Results,3)
colnames(UNI_Results) = c('HR','2.5% CI','97.5% CI','p')
# write all univariate and only the significant variables
write.csv(UNI_Results,'H:/Gerber/Timing_2_13_2017_ERNegUnivariateResults.csv')
write.csv(UNI_Results[UNI_Results[,4]<0.05,],'H:/Gerber/Timing_2_13_2017_ERNegSignificant_UnivariateResults.csv')
...
NODAL_ERNEG_Modified <- NODAL_ERNEG[(NODAL_ERNEG$INCOME_Recode !='Unknown') & (NODAL_ERNEG$EDUCATION_Recode != 'Unknown') ,]
...
+ as.factor(RX_SUMM_SURGICAL_MARGINS_Recode)
+ as.factor(FACILITY_TYPE_CD_Recode)
+ as.factor(ERSTATUS):as.factor(P_RT)
 + as.factor(HORM_RT_SEQ)
 + as.factor(EDUCATION_Recode)
 + as.factor(INCOME_Recode)
+ as.factor(INSURANCE_STATUS_Recode)
...
coxmodel <- coxph(Surv(followup_time, censor)~ as.factor(P_RT) + as.factor(Age_Recode):as.factor(P_RT) + as.factor(Age_Recode) 
+ as.factor(droplevels(TNM_PATH_T_Recode)):as.factor(P_RT) + as.factor(Age_Recode) 
+ as.factor(droplevels(TNM_PATH_N_Recode)):as.factor(P_RT) + as.factor(Age_Recode) 
+ as.factor(GRADE_Recode):as.factor(P_RT) + as.factor(GRADE_Recode) 
+ as.factor(RX_SUMM_HORMONE_Recode):as.factor(P_RT) + as.factor(RX_SUMM_HORMONE_Recode) 
...
# results = rbind(results,c(exp(coxmodel$coefficients[6]),exp(confint(coxmodel))[5,],table(NODAL_ERNEG_NoCHEMO$P_RT2)[2]))
# print(cutoff)
# }
...
# results = rbind(results,c(exp(coxmodel$coefficients[6]),exp(confint(coxmodel))[5,],table(NODAL_test$P_RT2)[2]))
# print(cutoff)
# }
