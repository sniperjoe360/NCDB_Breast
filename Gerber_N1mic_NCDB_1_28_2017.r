...
# With regard to univariate/multivariate model:

# 2) Grade must be redone since you are comparing well to moderate and poor to moderate and therefore gap between well and poor is not properly represented by the data (eg HR for grade is actually higher than for some of the other variables you list at the top but it is obscured by comparing to the middle variable instead of one of the extreme variables) 
# 3) PMRT is significant on UVA, no? not listed at the top. 
# I think we should get rid of "any systemic therapy" from all tables since we have chemo and hormone therapy as separate variables and I am not sure it adds anything.
