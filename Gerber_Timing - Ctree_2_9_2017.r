...
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(exp(confint(coxmodel)),coef(summary(coxmodel))[,5]))
...
+ as.factor(CROWFLY_Recode),data=NODAL_Modified)
...
+ as.factor(droplevels(RACE_Recode))
+ as.factor(CDCC_TOTAL_Recode)
+ as.factor(droplevels(TNM_PATH_T_Recode))
+ as.factor(TNM_PATH_N_Recode)
+ as.factor(HISTOLOGY_Recode)
+ as.factor(ERSTATUS)
+ as.factor(GRADE_Recode)
+ as.factor(HORM_RT_SEQ)
...
results = rbind(results,c(exp(coxmodel$coefficients[5]),exp(exp(confint(coxmodel))[5,],table(NODAL_test$P_RT2)[2]))
print(cutoff)
}
...
results = rbind(results,c(exp(coxmodel$coefficients[5]),exp(exp(confint(coxmodel))[5,],table(NODAL_ERNEG_NoCHEMO$P_RT2)[2]))
print(cutoff)
}
...
UNI_Results = rbind(UNI_Results,cbind(exp(coxmodel$coefficients),exp(exp(confint(coxmodel)),round(coef(summary(coxmodel))[,5],2)))
